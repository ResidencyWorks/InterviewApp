...
/**
 * Landing page component for the Interview Drills application
 * Magic link authentication page - checks for active session and redirects if authenticated
 */
export default function HomePage() {
	...
	useEffect(() => {
		setEmailId(`email-${Math.random().toString(36).slice(2, 11)}`);
	}, []);

	// Check for active session and redirect authenticated users to dashboard
	useEffect(() => {
		if (user && !authLoading) {
			console.log(
				"Landing page - Active session detected, redirecting to dashboard",
			);
