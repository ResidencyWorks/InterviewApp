...
export class EvaluationService implements EvaluationServiceInterface {
  ...
  async evaluate(request: EvaluationRequest): Promise<EvaluationResult> {
    ...
    try {
      ...
      // Perform evaluation
      const evaluation: EvaluationResponse = await this.engine.evaluate(request)
      ...
      try {
        ...
        console.error('Failed to update evaluation result with error:', dbError)
      }
    ...
    try {
      ...
      if (completedResults.length > 0) {
        const categorySums = completedResults.reduce(
          ...
          { clarity: 0, structure: 0, content: 0, delivery: 0 }
        )
