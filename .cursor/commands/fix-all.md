...
# Steps
1. Run `pnpm fix:all`. If command fails: capture error output, stop with failure status.
2. If passes, scan `.ts`/`.tsx` files for `: any` or implicit `any`.
   - Log each occurrence with file path and line number.
   - Optionally replace `: any` with `unknown` or a more specific type stub.
3. Re-run `tsc --noEmit` (or incremental build) to confirm no type errors remain.
4. Report summary: number of files modified, number of `any` instances replaced, number of remaining `any` occurrences requiring manual fix.
5. Respect `--dry-run`: in that mode skip writing changes, only print plan.
